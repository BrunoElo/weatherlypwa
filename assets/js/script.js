...
weatherWrapper.innerHTML = '<div class="weather__details"><img class="weather__icon" src="">            <p class="weather__description"></p>        </div><div class="current-temperature">            <p></p>        </div>    </div> '
...
sunTimes.innerHTML = '<div class="sunrise">            <p class="text-bold">Sunrise</p>            <p class="sunrise__time"></p>        </div>        <div class="sunset">            <p class="text-bold">Sunset</p>            <p class="sunset__time"></p>        </div>';
...
weatherElements.innerHTML = '<div class="cloudiness">            <p>Cloudiness</p>            <p class="cloudiness__value"></p>            <p>%</p>        </div>        <div class="humidity">            <p>Humidity</p>            <p class="humidity__value"></p>            <p>%</p>        </div>        <div class="pressure">            <p>Pressure</p>            <p class="pressure__value"></p>            <p>kPa</p>        </div>        <div class="wind">            <p>Wind</p>            <p class="wind__value"></p>            <p>km/h</p>        </div>';
...
if (weatherData != null) {
    ...
    receivingTime.innerHTML = new Date(weatherData.dt * 1000).toLocaleString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
    ...
    currentTemp.innerHTML = (weatherData.main.temp - 273.15).toPrecision(2) + '°C';

    sunrise.innerHTML = new Date(weatherData.sys.sunrise * 1000).toLocaleString('en-US', { hour: 'numeric', minute: 'numeric' });
    sunset.innerHTML = new Date(weatherData.sys.sunset * 1000).toLocaleString('en-US', { hour: 'numeric', minute: 'numeric' });
...
let weatherReq = () => {
    ...
    // Api request
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${searchValue}&APPID=cb5bf741dc48f795025eed310da9f370`).then((resp) => {
        ...
        receivingTime.innerHTML = new Date(data.dt * 1000).toLocaleString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
        ...
        sunrise.innerHTML = new Date(data.sys.sunrise * 1000).toLocaleString('en-US', { hour: 'numeric', minute: 'numeric' });
        sunset.innerHTML = new Date(data.sys.sunset * 1000).toLocaleString('en-US', { hour: 'numeric', minute: 'numeric' });
...
searchField.addEventListener('keyup', (event) => { // listen for the enter keypress
    if (event.keyCode === 13) {
        weatherReq();
    }
});
