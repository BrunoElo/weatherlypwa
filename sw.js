...
self.addEventListener('install', installEvent => {
    ...
    installEvent.waitUntil( //lets the installEvent function wait until the caching is done
...
self.addEventListener('activate', activateEvent => {
    ...
    activateEvent.waitUntil(
        caches.keys().then(keys => {
            console.log(keys);
            return Promise.all(keys.filter(key => key !== weatherlyApp && key !== dynamicWeatherlyApp).map(key => caches.delete(key)) //deletes old caches
            )
        })
    )
});
...
self.addEventListener('fetch', fetchEvent => {
    ...
    fetchEvent.respondWith(
        caches.match(fetchEvent.request).then(cachesRes => {
            return cachesRes || fetch(fetchEvent.request).then(fetchRes => { //returns cached resource if available else carries on with the fetch
                return caches.open(dynamicWeatherlyApp).then(cache => { // for inserting in dynamic cache
